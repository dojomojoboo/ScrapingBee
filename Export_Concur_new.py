...
async def extract_reviews(semaphore, company_name, num_pages):
    review_data = []
    url_list = []
    ...
    async with semaphore:
        async with aiohttp.ClientSession(headers={'User-Agent': 'Mozilla/5.0'}) as session:
            for page in range(1, num_pages + 1):
                ...
                response = await client.async_get(url, headers=headers)
                print(page)
                ...
                # Introduce a delay between requests (if necessary)
...
async def main():
    ...
    reviews = await extract_reviews(semaphore, company_name, num_pages)
